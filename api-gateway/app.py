...
app = Flask(__name__)
app.register_blueprint(driver_controller.driver_controller)
# configuration
# NEVER HARDCODE YOUR CONFIGURATION IN YOUR CODE
# INSTEAD CREATE A .env FILE AND STORE IN IT
# database name
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:Nintendo123@172.104.167.232:8201/account-call'
...
def edit_passenger_profile(current_user):
    ...
    if role == 'driver':
        response = requests.post(
            'http://127.0.0.1:5001/edit', json=request.json)
    elif role == 'passenger':
        response = requests.post(
            'http://127.0.0.1:5002/edit', json=request.json)
    else:
